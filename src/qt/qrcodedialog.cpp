...
{
    QString ret = QString("bitspace:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BSX unit input in lnReqAmount, we generate the URI with BSX as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(bitspaceUnits::format(bitspaceUnits::BSX, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
